        ...
        if( options.bare === false ){
            ...
            if( !( package_json_exists instanceof Error ) ) {
                await rename( './package.json', './_package.json' )
                    .catch( error => process_exit( error.message, Error( 'Object [react-dang.install] rename package.json' ), shell_exit_codes.internal ) )
            ...
            if( !( node_modules_exists instanceof Error ) ) {
                await rename( './node_modules', './_node_modules' )
                    .catch( error => process_exit( error.message, Error( 'Object [react-dang.install] rename node_modules' ), shell_exit_codes.internal ) )
            ...
            npmInstallReactDangApp.on( 'exit', async code => {
                ...
                removeDangApp.on( 'exit', async code => {
                    ...
                    install.on( 'exit', async code => {
                        ...
                        try{
                            delete package_json.engines
                            delete package_json.main
                            delete package_json.publishConfig
                            delete package_json.homepage
                            delete package_json.repository
                            delete package_json.bugs
                        }catch ( error ) {
                            console.warn( 'there was a problem while modifying package.json properties from @react-dang/app' )
                            console.warn( 'check it out after the installation is over and delete/edit the entries:' )
                            console.warn()
                            console.warn( 'engines' )
                            console.warn()
                            console.warn( 'main' )
                            console.warn()
                            console.warn( 'publishConfig' )
                            console.warn()
